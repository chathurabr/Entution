...
public class _05_06_TagASalesInquiryToALead {
	WebDriver driver;
	...
	public _05_06_TagASalesInquiryToALead(WebDriver driver){
		this.driver=driver;
	}
	
	public void TagASalesInquiryToALead(String inqAboutText,String budgetamount,String description,String employeeName){
		this.selectLead(inqAboutText, budgetamount, description, employeeName);
	}
	
	public void selectLead(String inqAboutText, String budgetamount, String description, String employeeName){
		...
		wait.until(ExpectedConditions.elementToBeClickable(leadAccDropdown));
		
		Select Leaddropdown = new Select(driver.findElement(leadAccDropdown));
		Leaddropdown.selectByVisibleText("Lead");
				Reporter.log("Lead option selected successfully");
		//
		...
		action.moveToElement(leadSrchicn).click().build().perform();
			...
			wait.until(ExpectedConditions.presenceOfElementLocated(popUpTxtBx));
